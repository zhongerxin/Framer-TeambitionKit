class exports.Circle extends Layer
	...
	constructor: (@options={}) ->
		...
		if @options.hasCounter isnt null
			...
			style = {
				...
				fontWeight: "#{@options.counterWeight}"
		...
		@.html = """
			<svg viewBox='-#{@options.strokeWidth/2} -#{@options.strokeWidth/2} #{@options.viewBox} #{@options.viewBox}' -webkit-filter='#{@options.dropShadow}' filter='#{@options.dropShadow}' >
				<defs>
		...
		@proxy.on 'change:x', ->
			...
			if self.options.hasCounter isnt false
				numberNow = Utils.round(self.proxy.x / 5)
				counter.html = numberNow
				if self.options.hasPercentage isnt false
					counter.html = numberNow + "%"
	...
	changeTo: (value, time) ->
		...
		if @options.hasCounter is true and @options.hasLinearEasing is false # override default "ease-in-out" when counter is used
			customCurve = "linear"
		else
